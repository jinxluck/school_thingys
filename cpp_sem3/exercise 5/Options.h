...
public:
	...
	int setOpts(string optstring);			// set's savedopt string
	int setOpts(char optchar[1]);				//sets an option char
	int numOpt();							// Returns number of compatible options given in argV
	int getop();							// shows what options have been saved
	int argsetter(int argC, char** argV);	// in case default constructer has been used
